...
const BitcoinHeader = ({ bitcoinData }: BitcoinHeaderProps) => {
  return (
    <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 mb-4 md:mb-6 bg-dashboard-medium/30 border border-border/30 dark:border-satotrack-neon/10 rounded-lg p-3 md:p-4 backdrop-blur-sm">
      <div className="flex items-center gap-3">
        <div className="bg-satotrack-neon/10 p-2 md:p-3 rounded-full border border-satotrack-neon/20">
          <Bitcoin className="h-5 w-5 md:h-6 md:w-6 text-satotrack-neon dark:text-satotrack-neon text-primary" />
        </div>
        <div>
      ...
      <div className="hidden md:block px-3 py-1 border border-border/30 dark:border-satotrack-neon/20 rounded bg-satotrack-neon/5 text-xs text-satotrack-neon font-mono self-start sm:self-auto">
        BTC-USD
      </div>
    </div>
  );
